...
def write_excel(mesh_dir, ts_entries, root):
    ...
    if any(d["fc_list"] for d in ts_data):
        ...
        _hdr(ws, 3, 1, "Statistic",   width=12)
        col = 2
        for d in ts_data:
            _hdr(ws, 2, col, d["label"], width=14)
            ws.merge_cells(start_row=2, start_column=col,
        ...
        for coeff_name, key in [("Cd", "Cd"), ("Cl", "Cl"), ("Cm", "Cm")]:
            for stat_name, fn in [("min", np.min), ("max", np.max), ("mean", np.mean)]:
                alt = r % 2 == 1
                c1 = ws.cell(row=r, column=1, value=coeff_name)
                c1.font = Font(bold=True, name="Arial", size=10); c1.border = BORDER
                ...
                for d in ts_data:
                    val = None
                    if d["fc_list"]:
                        arr = d["fc_list"][0].get(key, np.array([]))
                        if len(arr):
                            val = float(fn(arr))
                    _val(ws, r, col, val if val is not None else "N/A", alt)
                    col += 3
                r += 1
            r += 1
        ws.freeze_panes = "C4"
...
def main():
    ...
    for pp_dir in pp_dirs:
        ...
        if fc_s:
            print(f"    → forceCoeffs.png")
        if yp_s:
            print(f"    → yPlus.png  |  yPlus_min.png  |  yPlus_max.png  |  yPlus_mean.png")
        total_fc += fc_s
        total_yp += yp_s
    ...
    for mesh_dir, ts_entries in sorted(groups.items()):
        ...
        try:
            ...
            print(f"    → forceCoeffs_combined.png")
            print(f"    → yPlus_combined.png  |  yPlus_min_combined.png"
                  f"  |  yPlus_max_combined.png  |  yPlus_mean_combined.png")
            total_comb += 1
